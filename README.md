...
**Option A: Using the branded button**
  ...
  onComplete: (result) {
    switch (result) {
      case BushaPaySuccess(:final paymentId):
        // Payment completed — verify via webhook
...
**Option B: Headless (your own button)**
...
```bash
# iOS simulator
xcrun simctl openurl booted "com.example.myapp.busha-pay://callback?status=completed&paymentRequestId=PAYR_test&checkoutId=CHK_test"

# Android emulator
adb shell am start -a android.intent.action.VIEW \
  -d "com.example.myapp.busha-pay://callback?status=completed&paymentRequestId=PAYR_test&checkoutId=CHK_test"
```
...
|---|---|
| Option A or B (Flutter `Router`) | **`true`** — let Flutter deliver URLs to your router |
...
If you don't explicitly set the flag, the default is **`false`** on both platforms. Set to `true` only when you're using Flutter's built-in router.
...
1. The SDK opens a bottom sheet with the Busha checkout UI.
2. If the Busha app is installed, the user is redirected there to pay.
3. If the Busha app is not installed, the user pays in the web checkout.
...
| `sourceId` | `String?` | No[^1] | Checkout ID returned by the Busha create-charge API. Required to deep-link into the Busha app. |

[^1]: Optional for pure web-checkout flows, but **required** if you want "Pay with the Busha app" to deep-link into the installed Busha app. The Busha app uses `sourceId` to look up the pending checkout. Without it, tapping the Busha option in the checkout only shows the login/web flow.
