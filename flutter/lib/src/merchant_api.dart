...
/// `data.username` from the response — the same field pug-pay reads
/// server-side and renames to `business_name` for its chooser header.
///
/// Returns `null` on any failure (network error, non-2xx, missing
/// field) so callers can omit the merchant line silently.
Future<String?> fetchMerchantName(String publicKey) async {
  try {
